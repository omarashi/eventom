                ...
                @else
                    <div class="col-lg-4 order-left">
                        @if(count($ticket['upcoming'])>0)
                            <div>
                                <h5 class="mb-3">{{__('Upcoming Events')}}
                                    <button>{{count($ticket['upcoming'])}}</button>
                                </h5>
                                @foreach ($ticket['upcoming'] as $item)
                                    ...
                                    <div class="row event-data mb-3 pb-2 {{$loop->iteration==1?'active': ''}}" id="order-{{$item->id}}">
                                        ...
                                        <div class="col-9">
                                            <span class="badge text-white mr-2 {{$s}}">{{$item->order_id}}</span><span
                                                class="event-date">{{$item->event->start_time->format('D').', '.$item->event->start_time->format('d M Y')}}</span>
                        ...
                        @if(count($ticket['past'])>0)
                            <h5 class="mt-4 mb-3">{{__('Past Events')}}
                                <button>{{count($ticket['past'])}}</button>
                            </h5>
                                    ...
                                    <div class="col-5 ">
                                        <div class="right-data text-center">
                                            ...
                                            @if($review == null && $first_item->order_status=="Complete" || $first_item->order_status=="Cancel")
                                                <div>
                                                    <button class="btn open-addReview" data-toggle="modal" data-id="{{$first_item->id}}" data-order="{{$first_item->order_id}}"
                                                            data-target="#reviewModel"><i class="fa fa-star"></i></button>
                                                    <p>{{__('Review')}}</p>
                                                </div>
                                            @endif
                                            <div>
                                                <a href="{{ url('order-invoice-print/' . $first_item->id) }}" target="_blank" class="btn" id="print-btn"><i class="fa fa-print"></i>
                                                    <p>{{__('Print')}}</p></a>
                                            </div>
                                            @if($first_item->order_status != "Cancel")
                                                <div>
                                                    <form id="cancel-order-{{$first_item->id}}" action="{{ url('cancelOrder/' . $first_item->id) }}" method="post" style="display: none;">@csrf</form>
                                                    <a href="javascript:void(0)" class="btn" id="cancel-btn" onclick="event.preventDefault(); var confirm = window.confirm('Are You Sure ?'); if
                                                    (confirm) { $('#cancel-order-{{$first_item->id}}').submit(); }
                                                    "><i
                                                            class="fa fa-ban"></i>
                                                        <p>{{__('Cancel')}}</p></a>
                                                </div>
