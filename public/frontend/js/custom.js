			...
			success: function(result){
				if(result.success==true){
                    ...
                    if(result.data.order_status !== 'Cancel')
                        cancel_order = '<div><form id="cancel-order-'+result.data.id+'" action="/cancelOrder/'+result.data.id+'" method="post" style="display: none;"><input type="hidden"' +
                            ' name="_token" value="'+$('meta[name=\'csrf-token\']').attr('content')+'"></form><a' +
                            ' href="#" class="btn" id="cancel-btn" onclick="event.preventDefault(); var confirm = window.confirm(`Are You Sure ?`); if(confirm)' +
                            ' {$(\'#cancel-order-'+ result.data.id +'\').submit(); }"><i class="fa fa-ban"></i> <p>Cancel</p></a></div>';
					...
					<a class="btn" target="_blank" href="'+base_url+'/order-invoice-print/'+result.data.id+'"><i class="fa fa-print"></i></a><p>Print</p></div>'+cancel_order+'</div><div' +
                        ' class="payment-data hide" ><p class="mb-0"><span>Payment Method : </span>'+result.data.payment_type+'</p><p class="mb-1"><span>Payment Token : </span>'+payment_token+'</p><span class="badge '+payment_status_class+'">'+payment_status+'</span></div></div></div>');
